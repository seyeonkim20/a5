...
if args.function == 'pretrain':
    ...
    train_config = trainer.TrainerConfig(max_epochs=650, batch_size=128, learning_rate=6e-3, lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size, num_workers=4)
    trainer_GPT = trainer.Trainer(modle_gpt, pretrain_dataset, None, train_config)
    trainer_GPT.train()
    torch.save(modle_gpt.state_dict(), args.writing_params_path)
    # TODO [part f]:
    # - Given:
    ...
    if args.finetune_corpus_path is None:
        train_config = trainer.TrainerConfig(max_epochs = 75, batch_size = 256, learning_rate = 6e-4, lr_decay = True, warmup_tokens = 512*20, final_tokens = 200*len(pretrain_dataset)*block_size, num_worker = 4)
    else:
        modle_gpt.load_state_dict(torch.load(args.reading_params_path))
        modle_gpt = modle_gpt.to(device)
        train_config  = trainer.TrainerConfig(max_epochs=10, batch_size=256, learning_rate=6e-4, lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size, num_workers=4)
    finetune_lines = open(args.finetune_corpus_path).read()
    finetune_ds = dataset.NameDataset(pretrain_dataset, finetune_lines)
    trainer_GPT = trainer.Trainer(modle_gpt , finetune_dataset, None, train_config)
    trainer_GPT.train()
    torch.save(modle_gpt.state_dict(), args.writing_params_path)
    # TODO [part c] [part f]:
    # - Given:
